...
> **2026-09-14 附注（Lobby 消息附图，学生裁定落地）：** 聊天入口（`TutorLobbyInput`）开放学生主动附图（单条 ≤9 张），**保持 manifest 门禁**（不进入 agent-eligible 集合）：每一条带图请求仍构造 External manifest，披露集合按 schema 六的 `TUTOR_LOBBY_IMAGE_DISCLOSURE`（纯文本两类 + `SANITIZED_IMAGE_BYTES`/`IMAGE_DIMENSIONS`）逐请求精确相等，assets 必须与输入 `sourceImageAssetRefs` 的 id/sha/尺寸逐一相等；不带图的请求仍是纯文本最小披露。学生侧与拍照/讲题同口径（2026-09-14 二裁）：**加号选图 + 点发送本身就是本次知情，发起即发送、不弹确认卡、不再逐次询问**。图片字节在发送时登记进规范资产库（`LobbyMessageImageIntake`，相机与相册选择统一走 vault 字节校验），消息与图片的引用记入 `tutor_message_source_asset`（v44→45 迁移），孤儿清理按该表判定保留。消灭的失败：聊天里的图绕开披露清单出网、以及发出后没被引用的图片被后台清理掉。请求信封 schema v8→v9（新增 `sourceImageAssetRefs` 键，旧行指纹按 strip 平移），无模型或模型不支持看图时发送前即拒。
> **2026-09-14 附注（配图生成 / 去手写重绘两条链的出网条件）：** 这两条链不走 `ModelTaskRequest` 通道，因此没有 manifest 与进程内租约那一层；它们的出网条件是三样，缺一不发：①构建变体能力位 `networkRequestsAllowed`；②已配置、且能力测试确认支持图像输入的凭证（`resolveImageCredential` 与生成器内同一道门）；③**输入图片的字节必须来自规范记录并经逐位核对**——题面字节改经 `vault.resolve(record)` 读取（sha256 或字节数不符、文件缺失，一律返回 null），重绘也在真正读取时重新 `resolve` 一次。消灭的失败：磁盘上的题图被替换或损坏后仍被原样 POST——此前讲题会话的题面是按 URI 直接读文件，是图片链上最后一处「按路径取字节」的地方。**请求体上界（2026-09-14 核实，自我订正）**：这两条链发的是 **multipart 原始字节**，不是 base64，没有 4/3 膨胀，请求体 ≈ 源图字节 + 少量字段；源图来自规范资产（vault 导入上限 20 MiB），传输上限 36 MiB，因此**本来就不会超限**——此前说「缺总量预算」是把任务通道的 base64 预算套错了链路。真正缺的是把这条耦合写明：现在通道侧在组multipart 之前按**单资产出网上限 24 MiB**（`MODEL_EGRESS_MAX_ASSET_BYTES`）核对输入，超限即拒、一格字节不发（消灭的失败：来源上限将来被放宽后，请求会读完整块内存、组完请求才发现超限）。

- 当前生产 HTTP 边界已具备 HTTPS、公共地址检查、DNS 结果固定、禁用代理/重定向/自动重试、请求预算、超时及 401/429/5xx 映射（响应侧不设本地上限——大小由服务商限定；推理模型会把思考也流式发出，实测 395 字符答案伴随 3.2 MB 流，本地上限只会让合法答案失败）。Gateway 在打开任何图片前以 `Long` 计算 Base64 与 JSON 外壳的最坏上界，完整请求必须严格小于统一的 36 MiB transport 上限；总量超限、算术溢出或读取时大小变化都在 HTTP 前失败关闭。单题跨页使用 Room v9 有序来源束；多张照片可一次选择、逐张落盘并作为独立草稿稳定恢复。受限 Cartesian/SymbolTable 图形由客户端重建 ID、证据层并经结构校验后才允许屏幕/PDF 确定性绘制，复杂图继续失败关闭。仍未落地：用户选定 Provider 的线上识别评测、逐请求费用提示、像素内身份遮盖、可信裁切流、PDF/整卷批量分页、自动拆题、页级故障恢复和几何/物理/化学专用图形解析。
