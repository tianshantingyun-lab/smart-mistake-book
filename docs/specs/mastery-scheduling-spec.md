...
```
**指数是 `−1/w20`，不是 `1/w20`**（订正于 2026-09-13，审计 R-13）。它与 §2.2 的
`R(t,S) = (1 + FACTOR·t/S)^(−w20)` 互为反函数：把 `t = I(r*,S)` 代回即得 `R = r*`，
且 `I(0.9, S) = S` 逐位成立——这正是 §2.2 里 `FACTOR` 按 `R(S,S)=0.9` 定义的直接后果。
写成 `1/w20` 会让反解取错符号：`r*=0.8` 时给出 `−0.78·S`（**负间隔**），
`r*=0.9` 时给出 `−0.505·S`。实现（`FsrsScheduleMath.intervalDays`）一直是 `−1/w20`
（`decay = −w20`，代码里是 `pow(1.0 / decay)`）——**错的是这份文档**。
...
> **⚠ 上面的公式已被 §2.13 修订，且与实现分叉——照它去"修正"代码，会把对的改错。**
>
> - **分摊口径**：§2.13 已把默认改为「**全 KC 各记一次完整证据**」，`strength` 降级为排序／展示用途。
>   上面的 `strength_k / Σ_j strength_j` 只在「KC 学习曲线证明过度共现噪声大」时才启用，
>   **当前实现不启用**。研究依据见 `docs/research/behavior-signals-and-context-addendum.md` §5.2
>   （该修订本身带 UNVERIFIED 标注：Käser et al. 2013 一手核验未成功，"主流做法"依据降级为 DataShop 实践 + EDM 社区口径）。
>
> #### 2.8.1 归因是**写入时**的不可变事实（不变式，2026-09-12 增补）
>
> `attempt` 的 `attributions` 在**写入时**解析，并烘焙进不可变的 `assessment_evidence_snapshot`；
> 其逐字段哈希（`bindingId` / `knowledgeNodeId` / `weight` / `basisRevisionId` / `taxonomyVersion` /
> `role` / `certainty`）进入 `LearningLedgerFingerprint`，每条读取路径都会重算并与 `projection_outbox`
> 比对，不符即 `CONFLICT`、投影停摆。由此有三条约束：
>
> 1. **投影期不得读活的绑定表来解析归因。** 同一份事件日志必须在任何时刻重放出同一结果；
>    若投影期读活表，用户"今天"组织一次题目就会改变"昨天"的重放结果——「重放」从此是**调用时刻的函数**，
>    而不是真相的重建。
> 2. **改一条历史归因 = 改指纹 = 投影 `CONFLICT` 停摆。** 历史归因不能靠改写快照修。
> 3. **绑定表不是 append-only**（`ProblemOrganizationDao` 有硬 `DELETE FROM practice_unit_knowledge_binding`），
>    但它带 `NOT EXISTS(引用它的 attribution)` 守卫——**已经吃到证据的绑定不可删**。
>    这条守卫是上述不变式的运行时体现；放宽它等于放弃重放确定性。
>
> 三条合起来是一条定理：**能修历史的方案必然破坏重放确定性；保住重放确定性的方案必然修不了历史。**
> 要同时拿到两者，只有把"改归因"本身变成一种账本事件（选项 C）——而 `AttemptCorrection` 目前
> 只有"撤销刚才的结果"这一种语义，没有"改归因"的语义。
>
> 完整论证见 `docs/audit-2026-09-12-kernel-readiness.md` §3 **S-3**；决策 **D-2**＝「只修未来 + 文档化」。
> 落地形态：伪归因的写入层限定为「本题已存在 `sourceType != PSEUDO_FALLBACK` 的绑定时不落伪归因」。
> **推论（对实现者的方向指引）**：既然归因不可历史修复，那么**归因必须在写入时就是对的**——
> 新增任何"事后重映射到真 KC"的机制都是错的方向。
...
4. pseudo-KC 与真 KC 的迁移在题库增长期的爆炸半径（绑定数量级 10^2–10^3，可全量迁移）；
   > **2026-09-12 增补**：归因一旦落账就**不可修复**（见 §2.8.1 的不变式）——所以"迁移"只能是
   > **从此刻起不再写错**，不能回改历史。已知的错法：评级/自评证据**无条件**落到 `pseudo:<科目>`，
   > 有真绑定的题其真 KC 收不到这些证据（审计 `pseudo-attribution-unconditional`，`P1`）。
   > 处置＝写入层限定「本题已存在 `sourceType != PSEUDO_FALLBACK` 的绑定时不落伪归因」（决策 D-2、批 1 第 1.4 项）。
5. 优化器本地运行的计算预算（21 参数 × 万级日志，手机端秒级，可接受）；
6. 交互犹豫信号（RT 分位/修改次数/scroll-up）与时段乘数的样本门槛（个人基线 ≥30 次作答）在冷启动期不可用——启动期一律中性权重，避免小样本抖动；
7. **重建的确定性**（2026-09-12 增补，最高代价）：投影**不得**读活的绑定表来解析归因——
   `attributions` 是写入时烘焙进 `assessment_evidence_snapshot` 的不可变事实，其哈希进入账本指纹。
   违反它有两个后果，都不是"结果偏差"而是"机制失效"：①同一份事件日志在不同时刻重放出不同结果
   （重放从"真相的重建"退化为调用时刻的函数）；②任何对历史归因的改写都会让指纹比对失败、
   投影 `CONFLICT` 停摆。约束全文见 §2.8.1；推导见 `docs/audit-2026-09-12-kernel-readiness.md` §3 **S-3**。
...
- **同日语义（§2.15）**：以 py-fsrs short-term 调度器同构方式实现——`t==0` 走短程分支、`t>=1` 才落长程更新，满足"每卡每日至多一次长程更新"的 P0 不变量；learning_steps 保持禁用；review_log 保留全部原始证据（采集与调度解耦）。
  > **同日聚合条款的订正（2026-09-13，审计批 3 第 1 项；原文写"聚合无额外自由度"，该论证只对一半情形成立）。**
  >
  > §2.15 的这一条由**两句话**组成，而它们**并不唯一地合成一个算法**：
  > ①"同卡同日多条证据聚合成一条当日评级（G_agg = min(G_raw)，保守），日终/次日首开时落 FSRS 更新"；
  > ②"当日首学与**同日重复**走 short_term 分支"。
  > 若②照字面执行（同日每一条证据都走短程），则①里"日终落地的那一次更新"**没有独立的事件可以落**。
  > 实现选择了**照字面执行②**、并按①的**时点**（长程更新落在**次日首开**）而非①的**评级来源**
  > （该次复习自己的评级，而不是前一日全部证据的 `min`）实现。
  >
  > **因此真实的偏差只有一条**：跨日那一次更新用的评级**不是**前一日证据的 `min`。同日重复本身
  > **按②是规格要求的**，不是偏差。原文"聚合无额外自由度"的说法对**同日重复失败**成立
  > （每条 Again 即时衰减，实现比聚合更保守），对**同日重复成功**不成立——但**放大是有上界的**：
  >
  > 短程乘子 `m(S) = e^{w17·w18}·S^{−w19}`（G≥2 时钳制 `≥1`），其**不动点**
  > `S* = (e^{w17·w18})^{1/w19} = 2.1211` 天（默认参数 `w17=0.5425, w18=0.0912, w19=0.0658`，已实算）。
  > `S < S*` 时被推高、`S > S*` 时乘子 <1 被钳回 1.0，所以**任意多次同日成功最多把 S 推到 `max(S₀, 2.1211)`**
  > ——**与同日证据的条数无关**。实测（`S₀` 取首见 Again 的初始稳定度 0.212）：
  > 一次 Good → 0.2467；三次 → 0.3244（比"聚合成一次"高 **31.5%**，与审计 S-4 的实算一致）；
  > 40 次 → 1.8232；趋近上界 2.1211。相对"聚合成一次"的最坏倍率：
  > `S₀=0.212` → **8.60×**、`0.5` → 3.86×、`1.0` → 2.02×、`2.0` → 1.06×、`≥2.121` → **1.00×（无影响）**。
  > **换成用户看得见的东西**：保持率目标 0.9 时 `I(r*,S)=S`（§2.3 的已证性质），间隔按四舍五入取整且下限 1 天，
  > 所以这条偏差在调度上的最大后果是**首次间隔取 2 天而不是 1 天**——稳定度倍率看着大，间隔上只差一天。
  >
  > **学习科学一侧（沿用本仓已有的引用，非本次新增核实）**：间隔效应——同日内密集重复（massed）
  > 对长期保持的贡献远小于把这些重复摊到多日（spaced）——是本仓多处已引用的结论
  > （`KnowledgeReviewQueue.kt:103` 的 KDoc；带 DOI 的出处见
  > `docs/research/leech-remediation-research.md:79` 与 `docs/research/mastery-scheduling-research.md:120`，
  > Cepeda et al. 2006, *Psychol Bull* 132(3):354-380, DOI `10.1037/0033-2909.132.3.354`；
  > 另有 Kornell & Bjork 2008 的"学习者会系统性高估集中练习"
  > 见 `docs/research/batch-intake-research.md:23`）。这个方向**支持**"不要让同日成功
  > 大幅抬高 S"，而实现的上界 `max(S₀, 2.1211)` 恰好做到了这一点：成熟卡（S 已 > 2.12 天）
  > 在同日再答对**完全不加分**（倍率 1.00×）。**本次未新增对文献的一手核对**，因此这里只作为
  > 方向一致性的说明，不作为上面任何一个数的依据。
  >
  > **可达性（2026-09-13 核实）**：无任何不变量禁止同卡同日第二次记忆更新，唯一的闸门是防刷冷却
  > （§2.7：主观 6h、视觉 1h）；计划器**不排除未到期卡**（`ReviewPlannerV2.kt:488-496` 给未到期卡
  > 非零 `dueRisk`），故短队列下同日二次出题可达。**但要记一笔**：1h 冷却那条视觉通道
  > 目前**不在产品面上**——`docs/model-first-product-boundaries.md` 的 2026-09-06 结构化场景渲染隔离
  > 已使 `TUTOR_VISUAL_GENERATE`/`TUTOR_VISUAL_REVIEW` 不再触发、scene 不再渲染，
  > 于是 `VisualInteractionEventSink` 没有生产写入方。**当前实际可达的生产者是主观评级那条 6h 冷却**。
  >
  > **为什么不按①实现"日终聚合"**：①的"日终落地"需要**日界**这个概念，而事件溯源账本里
  > **没有日界事件**——投影由事件驱动、确定性重放，没有"今天结束了"这样一条输入。
  > 照字面实现要引入一个新的日界事件类型（或改成读侧聚合），并随之做**投影版本升级＋全量重放**，
  > 代价落在每一次现网安装上，换来的是"首次间隔 1 天而不是 2 天"。故本项按**订正说明**结清，
  > **保留现有行为**；若要改成规格字面语义，成本与影响面已由上述数字界定。
