...
- ~~知识点复习的作答判定回写掌握度：回写的权重/机制~~ → **已定**：对齐 T6 门控（本地算法做主——模型判正确/错误是客观信号，但回写权重/门控/配额由本地 `MasteryWriteGate` 决定，模型判断只是其中因素）。
- ~~分期~~ → **已定**：P1 先选择题（学生从选项作判断，不做口述）；P2 再做口述/简答打字判答。

> 规格自检（2026-09-06）：§6 原先的 4 个"待定"全部由用户确认收紧为"已定"。掌握度回写机制（对齐 T6 本地门控）、P1 只做选择题——两者均无歧义。无占位符、无内部矛盾、范围聚焦双复习入口。
