...
> **`MASTERY_READ` 扩容后的边界复核（2026-09-12，按上一条的逃逸条款执行）：** 该工具从"固定 24 行摘要"扩成"本科目按最弱优先的完整清单 ＋ 某个知识点的结构化历史聚合"，出网体量比原来大，因此显式复核它是否仍 ⊂ `RELEVANT_LEARNING_EVIDENCE`：**仍是**，理由是四条可核对的限定——
> 1. **科目内**：查询按 `node.subject = :subject` 过滤；科目是披露边界（不是"查完再裁"），跨科读取需要单独裁定，本次不做。
> 2. **节点粒度、无原始行**：返回的是每个知识点的投影摘要（保守掌握度、证据量、状态、时间档位）与**聚合计数**（独立答对次数/题目族数/学习日数、独立错误次数、讲题与测验证据的接受与被拒条数），**没有逐条事件行**——这正是"有界学习证据"与"完整学习历史"（`FULL_LEARNING_HISTORY`，永久禁止）的分界。
> 3. **无自由文本**：不含 `reason_markdown` 等模型自产文本（与"自由文本永远不是学习证据"一致），也不含题面或答案。
> 4. **时间只给档位**：沿用基线 evidence 的 5 档 `TutorEvidenceRecency`，且与基线共用同一份 `TutorEvidenceRecency.of`，避免两处渲染分叉。
>
> 另记两条本次新增的本地约束，它们限制的是**体量**而非类别：结果按字符预算截断并**显式注明**（对齐 spec §3.1"超限截断并注明 truncated"的原意，实现此前是抛错）；模型可对单次查询请求更大预算（`extendedResult`，语义而非数值），但每轮只放一次，上限是本地常量 `TutorToolOutcome.MAX_TOOL_RESULT_CHARS_EXTENDED`。

> **轮内总预算落地（2026-09-12，spec §3.1 另一半）：** 单工具上限管不到"一轮加起来"，所以补上了 `TutorToolRoundResult.MAX_TOOL_ROUND_RESULT_CHARS = 4_000`——三条默认 2k 结果原本会把 6k 塞进下一轮 prompt。它与扩展预算的关系是**相加**：某轮出现被放行的扩展调用时，该轮上限加 `EXTENDED_ROUND_RESULT_INCREMENT_CHARS = 4_000`，否则 4k 的轮门会让 6k 的单条结果永远到不了（那等于悄悄取消扩展预算）。
>
> 两条附带规定：**顺序分配**（先问的先得），**放不下的要说明自己**（截断追加标记；挤到连标记都没有则替换为"未随请求发送"），并且每一步都为后面每条预留那一行的额度——否则"说明自己被丢弃"本身会让总额反超刚设的门。压缩只改文本，不改 `ok`/`errorKind`（那描述的是工具是否执行，不是文本是否完整）。
>
> 常量放在工具环层而不是 `ModelTaskContractRegistry`：那张表**在生产路径上没有任何调用者**（仅测试锁定），把运行时门禁只写进它就成了"看着接好了其实没人喂"。`TutorToolLoop.kt` 与 `MAX_TOOL_RESULT_CHARS` 同处声明。
