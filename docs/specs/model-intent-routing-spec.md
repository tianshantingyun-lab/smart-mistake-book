...
### 9.7 T4 两段式的落地前置（2026-08-30 会话评估：暂缓）

T4 的 pending 持久化（提案表/确认 id 幂等/执行器）本轮评估为**不应先行实施**，链条上缺两环，缺一环都是死代码：

1. **无生产者**：授权矩阵按用户决策对 `notebook_write` 保持零声明（模型不可自主触发），只有"学生明确命令"的表达面（确认 UI 或命令入口）存在后才有合法触发源；先建提案表只会积压无人确认的死行。
2. **无消费者 + 执行器契约待定**：确认 UI 未建；确定性执行器的"增=新建草稿链路"需要 chat-origin 的 `CanonicalSourceAssetRecord` 归类——现有 capture 资产模型是图片导入中心的不可变事实层，扩展它属于合同变更，须用户批准后再动（§7 闭世界纪律）。

落地顺序建议：确认 UI/命令入口设计 → 用户批准 chat-origin 资产契约 → 提案表（v43+）与执行器一次做全（幂等键=确认 id）。
