...
> **指针（2026-09-25 补；2026-09-26 按第三轮实测更新一行，本报告历史结论一字未改）**：换大档**已于 2026-09-25 执行
> 并交裁**——本节的"Qwen3-Embedding-0.6B"已被 Stage-5 的选型取代为 **`bge-base-zh-v1.5`（同族同接口，768 维，MIT）**。
> Stage-5 落地判据（写死）= ①编码器对拍 ≥0.999 **且** ②真机单条编码 p50 ≤213ms（现役 71ms 的 3 倍）**且** ③门重定标只在①②都过后做（旧门不撤）。
> **第三轮实测**：①**过**——全量 28,931 行 docs 逐行 cosine 最小 **0.999127567**（queries 0.999171495），
> 金标融合主集 **0.7889（71/90）** ≥0.7444（净增 4 命中、0 丢失，逐章最小 0.4444）；
> ②**不过**——宿主同形态单条编码 p50 **16,843 ms**，按小档"宿主 2,545 ms → 真机 71 ms（≈36×）"的比例外推，
> base 档真机单条 **≈470 ms > 213 ms**（**外推，基档真机数不存在**）。
> ⇒ **未换档、随包侧逐字节保持 Stage-3 小档原样**（回退清单 **0 个文件**），判据③因此未触发。
> 全阶段证据、换件清单（含字节与 sha）、延迟硬线判定（判据②不达 / 未落地）与 UNVERIFIED 见
> **`docs/kb-stage5-report-2026-09-25.md`**；可复算的生成链与档位参数化见 `tools/dense_build/README.md` §8。
> 另记一条**外部阻塞**：本轮 Kotlin 侧验证（JVM 单测与仪器化）被另一条会话在 `core/database` 的飞行改动阻断
> （KSP `MissingType`），按纪律未碰其文件，状态记 **UNVERIFIED（外部阻塞）**（见 Stage-5 报告 §7 遗留 1）。
> **本节 §7.1/§7.2 的判据与表一律不动**（历史留档）。
