...
> **2026-09-13 追加**：本台账里与复习自评/四键评级相关的条目（自评映射、SELF_REPORT 来源）随之失效——两条通道已拆除，改由讲题判定（`MODEL_JUDGED_*`，非独立、HARD/AGAIN）承担；见 `model-judged-verdict-pricing.md`。
...
| 13 | 重教材料**类型优先级**改为显式策略并修正顺序（`MISCONCEPTION_GUIDE`/`WORKED_EXAMPLE` 优先，`COMPLETE_SOLUTION` 最后） | 审计 §3.9 的实体缺口。原顺序由 `KnowledgeTeachingMaterialDao.readForKnowledgeNodes` 的 `ORDER BY` 给出，该 CASE 把 `MISCONCEPTION_GUIDE` 排**最后**、`WORKED_EXAMPLE` 第四，与证据相反；而 `LIMIT` 在此决定哪些行进入 20k 字符预算，于是 leech 重教可能把预算花在泛泛讲解上、挤掉真正针对错误认知的材料。研究依据：Metcalfe 2017（纠正性反馈须分析错误推理）、Bokosmaty/Sweller/Kalyuga 2015（例题对新手有效）、Metcalfe 2025（"直接给正确答案+附材料"是被证明更差的一类）——见 `docs/research/leech-remediation-research.md` R5/R6 | `TutorTeachingReferenceSelector.reTeachPriority`（Kotlin 侧权威、可单测）+ 按 (role, priority, materialId) 排序；`KnowledgeTeachingMaterialDao` 的 CASE 同步（它只管截断，注释指明必须与 Kotlin 一致）；`ModelPromptPolicyVersions.TUTOR_PLAN` v10→v11、`TUTOR_RESPOND` v9→v10（材料顺序进了请求指纹） | `RoomTutorTeachingReferenceRepositoryTest` 6/6（新增 4 例，并改正了原断言——旧断言把传入顺序原样断言，对排序**完全覆盖不到**）；变异验证：把优先级反转使 3 个排序用例失败 |

| 14 | **leech 卡"先重教再练"开场**（补 §2.16 未实现的排期/界面形态） | 审计 §3.9 实体缺口的另一半：此前 leech 只有"降权 + 难度冻结"，学员第 7 次打开的还是那道连续失败 6 次的题，**没有任何重教发生**。spec §2.16 自述 leech「多为概念错」，不呈现对症材料就重复出题是在重复已被证明无效的动作。研究依据：Atkinson 2000（材料须与匹配练习**紧邻**）、Metcalfe 2025（须解释错误推理而非给答案），见 `docs/research/leech-remediation-research.md` R4/R5/R8 | `ReTeachInjection`（core:domain 纯策略，判定 + 选材）；`StudyExperienceRepository.reTeachOpening`（默认 null，不破坏既有实现）；`RoomBackedStudyExperienceRepository`（工件 `knowledgeNodeIds` 定范围、复用生产选择器取序）；`ReviewSessionScreen` + `ReviewSessionViewModel.reTeachAcknowledged`（确认穿过进程死亡后才露题干）；`TeachingArtifactLoad.reTeachOpening`（与工件同一次往返加载） | `ReTeachInjectionTest` 7/7、`RoomBackedStudyExperienceRepositoryTest` 19/19（新增 2 例）、`ReviewSessionViewModelTest` 6/6（新增 1 例）、`:app` 仪器化 `ReviewReTeachOpeningInstrumentedTest` 2/2 **在真机 AVD 上实跑通过**（含"确认前题干不存在"与"无 leech 直入题目"两个方向）；变异验证 3 处：去掉 leech 门 → 2 例阈值失败、去掉适用范围 → 1 例失败、材料反序 → 仓库用例失败 |

**改动 14 的已知边界**：开场只接在**有已校验教学工件**的复习项上（工件自带 `knowledgeNodeIds` 作检索范围）；**无工件**的自述/评级项（`CapturedReviewSessionScreen`）未接——那条路径在会话内拿不到知识点 id，需先解决来源问题。§2.9 的前置补救已由改动 15 接线。

| 15 | **§2.9 前置补救接线**：前置图喂入排程请求 + 会话注入前置 KC 的材料（非阻塞） | 审计与本次核实共同确认：`ReviewPlanningRequest.knowledgePrerequisites` **从未被生产调用点赋值**，§2.9 的前置门恒等于"无前置"——`prereqGap` 结构性为 0、`PREREQ_GAP` 理由从不产生、`PREREQ_GAP_WEIGHT × gap` 是死算术，§6/C3 的易混对通道同源失效。研究 R9 要求注入**前置 KC** 的材料且与常规排期**并行**（不阻塞） | `KnowledgeReadiness`（core:domain，判定唯一权威：阈值 + 最弱阻塞前置，含"未知≠不会"与同弱按 id 定序）；`KnowledgePrerequisiteReader`（core:data：按 KC 自己科目分区、256 分块、取回前置节点行）；`StudyReviewPlannerService` 喂图；`PrerequisiteRemediationPolicy` + `StudyExperienceRepository.prerequisiteRemediation` + `RoomBackedStudyExperienceRepository`；`ReviewSessionScreen` 的 `review_prereq_remediation` 卡（与题干并存，不设确认门）；`TeachingReferenceMarkdown`（两条通道共用渲染，消除分叉） | `KnowledgeReadinessTest` 9/9、`PrerequisiteRemediationPolicyTest` 5/5、`ReviewPlannerV2Test` 15/15（新增 2 例）、`KnowledgePrerequisiteReaderTest` 6/6、`RoomBackedStudyExperienceRepositoryTest` 26/26（新增 6 例）、`:app` 仪器化 `ReviewPrerequisiteRemediationInstrumentedTest` 2/2 **在真机 AVD 上实跑通过**（含"材料与题干并存、提交按钮可用"与"无缺前置不弹卡"两个方向）。变异验证 4 处：停喂前置图（复现修复前状态）→ 端到端用例失败；降权项置零 → 排程用例失败；把未知前置当 0 → 2 例失败；材料检索改用题目科目 → 科目用例失败；分块上限抬到 4096 → 分块用例失败；把补救卡改成阻塞 → 仪器化用例失败

**改动 15 的已知边界**：知识点复习队列本身（`selectKnowledgeReviewQueue`）**未**纳入前置判定——`KnowledgeReviewCandidate` 没有前置维度，spec 也没规定"ready 门是否该扩到出题对象"，属另一次设计决定。
...
| §3.9 leech 只在 V2 生效 | **V1 不可达——不是活缺陷；重教选材优先级已修（2026-09-11）** | V1 的 `plan()` 只出现在 `StudyReviewPlannerService.kt:309-318` 的 `else` 分支，而 `useReviewPlannerV2` 的默认值是编译期常量 `true`（`RoomBackedStudyExperienceRepository.kt:946`），全仓**没有任何调用方传 false**（已 grep 核实）——不存在可翻转的运行时开关，故"kill-switch 回退时 leech 静默失效"不成立，给死代码补 leech 检查不消除任何真实失败。注意 V1 的 `scoreKnowledgeNode` 是**活**路径（`KnowledgeReviewQueue` 用），但 leech 是题级记忆概念，知识点本就没有该状态。真正落地的缺口是**重教材料选材优先级与证据相反**（本文件改动 13）；同轮核实修正三处文档失实：`docs/research/three-store-linkage-design.md` §3.6（role 是标签、无权重；"∪ 其前置"未实现；类型优先级权威在 Kotlin 侧）与 `docs/specs/mastery-scheduling-spec.md` 核心状态机行的"planner 剔除常规排期"（实为 ×0.15 降权，硬排除会自锁恢复路径）。**2026-09-11 续修**：该条的另一半——"强制注入 teaching_material 重教"的排期/界面形态——已实现（本文件改动 14），带三条已知边界 |
