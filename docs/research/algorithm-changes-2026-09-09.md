...
| §3.9 leech 只在 V2 生效 | **V1 不可达——不是活缺陷；重教选材优先级已修（2026-09-11）** | V1 的 `plan()` 只出现在 `StudyReviewPlannerService.kt:309-318` 的 `else` 分支，而 `useReviewPlannerV2` 的默认值是编译期常量 `true`（`RoomBackedStudyExperienceRepository.kt:946`），全仓**没有任何调用方传 false**（已 grep 核实）——不存在可翻转的运行时开关，故"kill-switch 回退时 leech 静默失效"不成立，给死代码补 leech 检查不消除任何真实失败。注意 V1 的 `scoreKnowledgeNode` 是**活**路径（`KnowledgeReviewQueue` 用），但 leech 是题级记忆概念，知识点本就没有该状态。真正落地的缺口是**重教材料选材优先级与证据相反**（本文件改动 13） |
