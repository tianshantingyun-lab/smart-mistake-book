...
> **2026-08-01 最新覆盖：** 底栏顺序为 `讲题 / 复习 / 错题本 / 我的`，冷启动进入讲题聊天。后文历史冻结段中的旧顺序只保留追踪用途。

状态：治理基线（不等于运行时验收报告）  
日期：2026-07-22  
适用范围：产品决策、实现合同、体系蓝图、信息架构与 `design-qa.md`

> **A04 机器校验映射：** 每个关键场景的入口、用户可见行为、数据变化、重试策略和测试编号统一登记在
> `tools/scenario_to_test_registry.json`，由 `tools/tests/test_scenario_mapping.py` 自动校验
> 唯一 ID、必填字段、禁用占位文案，以及 `Class#method` 是否真实存在于测试/仪器测试源码。
...
| [`../design-qa.md`](../design-qa.md) | 带日期的视觉与交互回归记录 | 证明真实数据闭环、数据库故障恢复或 M1 退出门禁 |
