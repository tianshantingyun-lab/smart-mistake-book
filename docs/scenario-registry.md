...
| `ReviewSession` | 复习会话用例 | `startOrResumeReviewSession()`；curated 选择题走 `submitReviewChoice()`；真实保存题（无机判选项）走**讲题判定结算** `settleTutorJudgedReview()`——原先的 `submitReviewSelfReport()`/`submitReviewRating()` 两条主观通道已于 2026-09-13 拆除（产品裁定：复习作答与评级不再由学生决定；定价依据 `docs/research/model-judged-verdict-pricing.md`）。真实题页面只显示数据库保存的精确原题修订，不生成另一道题或虚构答案；讲题会话里学生每一轮文字都会先落 `tutor_message`（STUDENT 行），模型可以出**开放式检查题**（一句话提问、学生自己组织语言回答），其正向判断必须逐字引用学生原话/作答——引文由本地逐条比对，连一条已核实引文都没有的正向会被拒写（2026-09-13 起对**所有**模型交互生效，不只复习结算）；**没有可用模型时（strictOffline，或未配置/配坏 provider）该题保持阻塞、如实提示**——这是 2026-09-14 知情裁定的取舍（另两个候选：零权重「本次先跳过」需新增账本语义；计划期排除会让 strictOffline 的复习队列直接变空）。**已知代价：该状态下整条复习队列都不推进**（生产线上的题都是无工件题，第 1 题就卡住），因此无模型构建等于没有复习功能；“这里还卡住”推进当前复习后进入同一题同一修订的讲题 | 两条提交路径都必须在单一 Room 事务中写入精确 Attempt/receipt 并 CAS 推进；UI 持久化首次完整命令，失败或重建后精确重放，未知结果时阻止重复改报 | `IMPLEMENTED-SLICE`；选择提交与真实题自报的成功、回滚、幂等/重建已有专项覆盖；仍缺真实规模和物理设备强杀矩阵 |
...
- 模型配置响应式状态修正后，真实手指路径完成“我的 → 智能能力 → 保存 → 返回已保存状态 → 清除 → 返回未配置状态”；API Key 在控件树中无明文。应用级设备回归按唯一模拟器序列分别重跑，`localFirstDebug 10/10` 与 `strictOfflineDebug 10/10` 均通过。全工程 JVM 测试、双 flavor Lint 与双 APK 构建同轮 `478` 个 Gradle 任务成功；OpenAI-compatible 网关专项测试再次通过，并把 HTTP/SSRF/超时独立为传输边界。
