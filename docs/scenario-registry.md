...
| `ReviewSession` | 复习会话用例 | `startOrResumeReviewSession()`；curated 选择题走 `submitReviewChoice()`；真实保存题（无机判选项）走**讲题判定结算** `settleTutorJudgedReview()`——原先的 `submitReviewSelfReport()`/`submitReviewRating()` 两条主观通道已于 2026-09-13 拆除（产品裁定：复习作答与评级不再由学生决定；定价依据 `docs/research/model-judged-verdict-pricing.md`）。真实题页面只显示数据库保存的精确原题修订，不生成另一道题或虚构答案；“这里还卡住”推进当前复习后进入同一题同一修订的讲题 | 两条提交路径都必须在单一 Room 事务中写入精确 Attempt/receipt 并 CAS 推进；UI 持久化首次完整命令，失败或重建后精确重放，未知结果时阻止重复改报 | `IMPLEMENTED-SLICE`；选择提交与真实题自报的成功、回滚、幂等/重建已有专项覆盖；仍缺真实规模和物理设备强杀矩阵 |
