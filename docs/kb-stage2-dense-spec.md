...
- **按 §1.2 的三种归宿**：落在**第 3 种**（主判据不过 → **不立项**；这 90 条里 dense 也捞不回的题归入内容/绑定问题，不在检索器上继续加机制）。
  > **指针（2026-09-24 用户改判，不改本段结论）**：用户当日裁定 **dense 立项、先小档跑通端侧闭环**（bge-small-zh-v1.5 int8 + LiteRT），闭环达标后再决定是否换大档。上表的两条判据**未被改判、也未被满足**（0.75/0.60 一字未动）；「不立项」是 Stage-2 判读时的归宿，**已被用户改判取代**。Stage-3 的端侧落地与真机数见 `docs/kb-stage3-report-2026-09-24.md`；判定模板与形态预案同步见 `docs/kb-vector-topic-decision.md` §3.2 现状段 2026-09-24 行。
