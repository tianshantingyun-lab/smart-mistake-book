...
fun CapturedReviewSessionScreen(
     ...
     * 摆在题干旁。与策展屏同一处摆放、同一条规则：**不拦作答**，没有确认按钮（审计批 2 第 3 项）。
     */
    prerequisiteRemediation: PrerequisiteRemediation? = null,
    /**
     * 开场重教（spec §2.16）：leech 卡先呈现针对错误认知的材料，**学员确认后才露出题干**。
     * 与 [prerequisiteRemediation] 的差别是**扣不扣题**——重教是进入本次作答的必经步骤（审计 N-16）。
     */
    ...
    RootPageColumn(modifier = modifier.testTag("captured_review_session_root")) {
        ...
        // 本屏在第 2 条之后是**无状态**的（不再有 ViewModel），所以确认状态用 rememberSaveable：
        // 它同样穿过进程死亡，否则重建会把学员按回材料页、或反之绕过重教直接看题。
        var reTeachAcknowledged by rememberSaveable { mutableStateOf(false) }
        val opening = reTeachOpening
        if (opening != null && !reTeachAcknowledged) {
            PaperDivider(Modifier.padding(vertical = 18.dp))
            ReTeachOpeningCard(
                opening = opening,
                onAcknowledge = { reTeachAcknowledged = true },
            )
