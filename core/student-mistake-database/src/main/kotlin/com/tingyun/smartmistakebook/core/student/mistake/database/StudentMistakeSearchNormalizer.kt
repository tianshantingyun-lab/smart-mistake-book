...
internal data class NormalizedStudentProblemSearchDocument(
    val normalizedText: String,
    val tokenizedText: String,
    val sourceCanonicalFingerprint: String,
)

internal object StudentMistakeSearchNormalizer {
    fun document(
        revisionId: String,
        title: String?,
        stemMarkdown: String,
        practiceUnitTitle: String,
    ): NormalizedStudentProblemSearchDocument {
        val normalizedText =
            normalize(
                listOfNotNull(title, stemMarkdown, practiceUnitTitle)
                    .joinToString("\n"),
            )
        return NormalizedStudentProblemSearchDocument(
            normalizedText = normalizedText,
            tokenizedText = tokenize(normalizedText),
            sourceCanonicalFingerprint =
                CanonicalSha256(SEARCH_SOURCE_FINGERPRINT_DOMAIN)
    ...
    fun query(raw: String): NormalizedStudentProblemSearchQuery {
        ...
        return NormalizedStudentProblemSearchQuery(
    ...
    private const val SEARCH_SOURCE_FINGERPRINT_DOMAIN =
        "student-problem-search-source-v1"

    private fun String.isCjkToken(): Boolean =
        codePoints().allMatch(::isCjkCodePoint)
}

internal data class NormalizedStudentProblemSearchQuery(
    val normalizedText: String,
