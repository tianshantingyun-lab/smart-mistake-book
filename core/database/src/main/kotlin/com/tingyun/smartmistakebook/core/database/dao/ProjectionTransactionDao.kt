...
internal abstract class ProjectionTransactionDao {
    ...
    protected abstract suspend fun findProjectionLearningObservationAdmission(
        eventId: String,
    ...
    protected abstract suspend fun findProjectionLearningObservationProof(
        sourceFactId: String,
    ...
    open suspend fun loadLearningLedger(learnerId: String): LearningLedgerRead {
        ...
        for (row in findAllOutbox(learnerId)) {
            ...
            prefix += PersistedLearningLedgerEvent(
                event = event,
                canonicalFingerprint = LearningLedgerFingerprint.event(event),
            )
            expected++
    ...
    private suspend fun verifyAppliedEventWindows(snapshot: LearnerSnapshot, learnerId: String) {
        ...
        snapshot.appliedLearningObservationRecords.values.forEach { applied ->
            ...
            val admitted = readLearningObservation(observation.toOutbox())
            if (observation.learnerId != learnerId ||
                observation.eventSequence != applied.eventSequence ||
                admitted !is AdmittedLearningObservationEvent ||
                LearningLedgerFingerprint.admittedLearningObservation(admitted) !=
                    applied.canonicalFingerprint
            ) {
                throw ProjectionCasConflictException(
    ...
    private suspend fun ProjectionOutboxEntity.projectionReceiptFingerprint(): String? =
        when (eventKind) {
            EVENT_KIND_LEARNING_OBSERVATION ->
                readLearningObservation(this)?.let(LearningLedgerFingerprint::event)

            else -> canonicalFingerprint.takeIf { hasValidCanonicalEvent() }
        }
    ...
    private suspend fun readLearningObservation(
        ...
        val sourceFactId = entity.sourceFactId ?: return event
        val admission = findProjectionLearningObservationAdmission(entity.eventId) ?: return event
        val proof = findProjectionLearningObservationProof(sourceFactId) ?: return event
        if (
            !LearningObservationSourceFactProofFingerprint.isValid(proof) ||
            admission.eventId != entity.eventId ||
            admission.sourceFactId != sourceFactId ||
            admission.rawEventCanonicalFingerprint != entity.canonicalFingerprint ||
            admission.sourceFactProofFingerprint != proof.proofFingerprint ||
            proof.sourceFactId != sourceFactId ||
            proof.learnerId != entity.learnerId ||
            proof.subject != entity.subject ||
            proof.targetKind != PROJECTION_TARGET_KIND_COMMITTED_PRACTICE_UNIT ||
            proof.targetDatabase != PROJECTION_TARGET_DATABASE_MISTAKE_COLLECTION ||
            proof.targetId != entity.practiceUnitId ||
            proof.targetVersion != entity.problemRevisionId ||
            proof.attestedAtEpochMillis > admission.admittedAtEpochMillis ||
            admission.admittedAtEpochMillis != entity.confirmedAtEpochMillis
        ) {
            return event
        }
