...
import com.tingyun.smartmistakebook.core.data.model.isPubliclyRoutable
...
private class OkHttpKnowledgeResearchTransport : KnowledgeResearchHttpTransport {
    override suspend fun get(sourceUri: String): KnowledgeResearchHttpResponse {
        val endpoint = PublicKnowledgeResearchEndpoint.resolve(sourceUri)
        val client = OkHttpClient.Builder()
            .dns(PinnedResearchDns(endpoint.host, endpoint.addresses))
...
private data class PublicKnowledgeResearchEndpoint(
    val url: HttpUrl,
    val host: String,
    val addresses: List<InetAddress>,
) {
    companion object {
        suspend fun resolve(sourceUri: String): PublicKnowledgeResearchEndpoint =
            withContext(Dispatchers.IO) {
                val url = sourceUri.toHttpUrlOrNull()
                    ?: throw UnsafeKnowledgeResearchEndpointException()
                if (
                    url.scheme != "https" ||
                ...
                PublicKnowledgeResearchEndpoint(
                    url = url,
                    host = url.host,
                    addresses = addresses.distinctBy { address -> address.hostAddress },
                )
            }
    }
}

internal class UnsafeKnowledgeResearchEndpointException : IllegalArgumentException()

private class PinnedResearchDns(
    ...
    override fun lookup(hostname: String): List<InetAddress> {
        if (!hostname.equals(approvedHost, ignoreCase = true)) {
            throw UnknownHostException("Unexpected research source host")
        }
