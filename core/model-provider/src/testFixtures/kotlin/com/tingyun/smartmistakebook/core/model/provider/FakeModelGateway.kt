...
/** Test fixture for task persistence and GUI-state integration tests. */
class FakeModelGateway(
    ...
    override fun execute(execution: ModelGatewayExecution): Flow<ModelGatewayEvent> = flow {
        ...
        emit(
            ModelGatewayEvent.Progress(
                ...
                userMessage =
                    if (request.input is CaptureParseInput) {
                        "正在演示印刷与手写分区步骤"
                    } else {
                        "正在读题"
                    },
    ...
    private fun outputFor(request: ModelTaskRequest) = when (val input = request.input) {
                ...
                assessment =
                    CaptureAssessment(
                        ...
                        followingPageRelations =
                            List(input.followingSourceAssets.size) { CapturePageRelation.UNSURE },
                    ),
            )
                                ...
                                QuestionBlockEvidence(
                                    ...
                                    sourceRegion =
                                        source.selectedRegion
                                            ?: NormalizedSourceRegion(0.0, 0.0, 1.0, 1.0),
        ...
        is TutorOpenResponseEvaluationInput ->
            error("The capture-only demo provider cannot evaluate open responses")
        is TutorRespondInput -> error("The capture-only demo provider cannot answer tutor messages")
        is TutorVisualGenerateInput ->
            error("The capture-only demo provider cannot generate tutor visuals")
        is TutorVisualReviewInput -> error("The capture-only demo provider cannot review tutor visuals")
        is ProblemOrganizationInput ->
            error("The capture-only demo provider cannot organize committed problems")
        is ProblemOrganizationV3Input ->
            error("The capture-only demo provider cannot organize image-grounded problems")
    }
