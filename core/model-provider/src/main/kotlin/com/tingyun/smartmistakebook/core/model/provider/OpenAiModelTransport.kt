...
internal class OkHttpModelTransport : StreamingModelHttpTransport {
    override suspend fun post(
        ...
        val endpoint = PublicHttpEndpoint.resolve(baseUrl)
...
internal data class PublicHttpEndpoint(
    val url: HttpUrl,
    val host: String,
    ...
    companion object {
        suspend fun resolve(
            baseUrl: String,
            requireDefaultPort: Boolean = false,
        ): PublicHttpEndpoint = withContext(Dispatchers.IO) {
            val base = baseUrl.toHttpUrlOrNull()
                ?: throw UnsafeModelEndpointException()
            if (
                base.scheme != "https" ||
                base.username.isNotEmpty() ||
                base.password.isNotEmpty() ||
                (requireDefaultPort && base.port != 443)
            ) {
                throw UnsafeModelEndpointException()
            }
